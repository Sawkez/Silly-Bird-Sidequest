...
struct Tile {
	...
	static const int DRAW_DEST_OFFSET = (WorldConstants::TILE_SIZE_F - DRAW_SOURCE_SIZE) / 2.0;
	...
	void Draw(SDL_Surface* targetSurface, const vector<SDL_Surface*>& atlases, int xOffset, int yOffset) {
		...
		SDL_Rect destination{x * WorldConstants::TILE_SIZE + DRAW_DEST_OFFSET + xOffset, y * WorldConstants::TILE_SIZE + DRAW_DEST_OFFSET + yOffset,
